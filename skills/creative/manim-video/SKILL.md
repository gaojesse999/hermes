...
**Ubuntu/Debian gotcha:** a plain `pip install manim` can fail while building `pycairo` / `manimpango` if system headers are missing. Install these first:
...
If your scene uses only `Text` and avoids `MathTex`/LaTeX formulas, you can often render without installing LaTeX. For formula-heavy scenes, keep LaTeX installed as originally recommended.
